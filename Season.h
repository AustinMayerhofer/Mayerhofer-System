...
public:
	...
	Season(std::string team_file, std::string scores_file, int week_num);
	~Season();
...
private:
	...
	void read_scores_NFL(std::string file_name);
	...
	void create_game_NFL(std::istringstream& stream, std::string token, char delimiter);
	...
	void read_game_NFL(std::string& winner_name, int& winner_score, std::string& loser_name, int& loser_score, std::istringstream& stream, std::string token, char delimiter);
